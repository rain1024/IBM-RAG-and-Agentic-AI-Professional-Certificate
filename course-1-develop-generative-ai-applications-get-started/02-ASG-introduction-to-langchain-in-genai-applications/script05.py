...
"""Demo 3: Another Few-Shot Example - Language Translation"""
print("\n=== Demo 3: Few-Shot Language Translation ===")

# Define examples for English to Spanish translation
...
# Create template for translation examples
translation_example_prompt = PromptTemplate(
    input_variables=["english", "spanish"],
    template="English: {english}\nSpanish: {spanish}"
)

# Create few-shot prompt for translation
translation_few_shot_prompt = FewShotPromptTemplate(
    examples=translation_examples,
    example_prompt=translation_example_prompt,
    prefix="Translate the following English phrases to Spanish:",
...
# Test translation
test_phrase = "I love learning languages"
translation_prompt = translation_few_shot_prompt.format(input=test_phrase)
...
response = llm.invoke(messages)
print(f"\nTranslation Response: {response.content}")
